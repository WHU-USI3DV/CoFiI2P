...
s
    ...
    Args:
        ...
        upsample_indices: [n2, max_num] Only the first column is used for pooling [5120, 512]

    Returns:
        x: [n2, d] pooled features matrix
    """
    # Add a last row with minimum features for shadow pools
    x = torch.cat((x, torch.zeros_like(x[:1, :])), 0)  # [2561, 2048]
    # Get features for each pooling location [n2, d]
    # print(upsample_indices[:, 0].max())
