...
from .modules import ConvBlock, ResidualBlock, UnaryBlock, LastUnaryBlock, nearest_upsample


class KPConvFPN(nn.Module):
    def __init__(self, input_dim, output_dim, init_dim, kernel_size, init_radius, init_sigma, norm,group_norm):
        super(KPConvFPN, self).__init__()

        self.encoder1_1 = ConvBlock(input_dim, init_dim, kernel_size, init_radius, init_sigma, norm, group_norm)
        self.encoder1_2 = ResidualBlock(init_dim, init_dim * 2, kernel_size, init_radius, init_sigma, norm,group_norm)
    ...
    def forward(self, data_dict):
        ...
        latent_s5 = feats_s5
        feats_list.append(feats_s5)  # [2560, 2048]
...
if __name__ == '__main__':
    network = KPConvFPN(input_dim=1, output_dim=256, init_dim=64, kernel_size=15, init_radius=4.25*0.3, init_sigma=2*0.3, group_norm=32)