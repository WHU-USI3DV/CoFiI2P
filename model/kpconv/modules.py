...
def get_norm(norm,channels,num_groups = None):
    if norm == "bn":
        return nn.BatchNorm1d(channels)
    elif norm == "ln":
        return nn.LayerNorm(channels)
    elif norm == "gn":
        assert num_groups > 1, "number of groups must be positive number!"
        return GroupNorm(num_groups,channels)
    else:
        raise ValueError("only support batch normalization, layer normalization and group normalization now!")


class UnaryBlock(nn.Module):
    def __init__(self, in_channels, out_channels, norm, group_norm, has_relu=True, bias=True, layer_norm=False):
        ...
        self.norm = get_norm(norm,out_channels,num_groups=group_norm)
        if has_relu:
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        # else:
        #     self.norm = GroupNorm(group_norm, out_channels)
        self.norm = get_norm(norm,out_channels,group_norm)
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        if in_channels != mid_channels:
            self.unary1 = UnaryBlock(in_channels, mid_channels, norm, group_norm, bias=bias, layer_norm=layer_norm)
        else:
        ...
        self.norm_conv = get_norm(norm,mid_channels,group_norm)

        self.unary2 = UnaryBlock(
            mid_channels, out_channels, norm, group_norm, has_relu=False, bias=bias, layer_norm=layer_norm
        )

        if in_channels != out_channels:
            self.unary_shortcut = UnaryBlock(
                in_channels, out_channels, norm, group_norm, has_relu=False, bias=bias, layer_norm=layer_norm
            )
        else:
